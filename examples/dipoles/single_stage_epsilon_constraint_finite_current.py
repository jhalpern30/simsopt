...
# SIMSOPT imports
from simsopt.geo import SurfaceRZFourier, curves_to_vtk, RotatedCurve, CurvePlanarFourier
...
parser.add_argument(
    ...
    type=float,
    default=0.0,
    help="Plasma current filament current [A]. Set nonzero to include finite-I Boozer term.",
)
...
PLASMA_CURRENT = _args.plasma_current
THETA_TOL = 0.01
...
def fun(x):
    ...
    # Run boozer surface
    res = run_boozer_code(boozer_surface, run_dict['iota'], run_dict['G'], run_dict['I_plasma'])
...
def run_boozer_code(boozer_surface_obj, iota_guess, G_guess, I_guess):
    """
    Branch-compatible Boozer call:
    - finite-current branch: run_code(iota, G, I)
    - legacy behavior: run_code(iota, G)
    """
    try:
        return boozer_surface_obj.run_code(iota_guess, G_guess, I_guess)
    except TypeError:
        return boozer_surface_obj.run_code(iota_guess, G_guess)


def plot_cross_section_with_plasma_filament(
    surf,
    VV,
    output_dir,
    label,
    plot_config,
    plasma_filament_radius,
    base_dipole_coils=None,
):
    """
    Cross-section diagnostic plot with a marker showing plasma filament location.
    """
    fig, ax = plt.subplots(figsize=(7, 6))
    phi_array = np.linspace(0, 0.5 / surf.nfp, 6, endpoint=True)
    for phi_slice in phi_array:
        ...
        ax.plot(rs, zs)
        ax.plot(rs2, zs2, "k")
        ax.plot(np.mean(rs), np.mean(zs), "kx")
    if base_dipole_coils:
        for coil in base_dipole_coils:
            ...
            if len(crossings) >= 2:
                ...
                R0c, Z0c = crossings[0]
                R1c, Z1c = crossings[-1]
                ax.plot([R0c, R1c], [Z0c, Z1c], c="tab:orange", linewidth=1.5, alpha=0.9, zorder=5)
            elif crossings:
                ax.plot(crossings[0][0], crossings[0][1], "o", c="tab:orange", markersize=4, zorder=5)
    ...
    ax.tick_params(axis="both", which="major", labelsize=plot_config.ticklabelfontsize)
    ax.set_aspect("equal", adjustable="box")
    fig.tight_layout()
    fig.savefig(os.path.join(output_dir, f"x_section_{label}.png"), dpi=plot_config.dpi, bbox_inches="tight")
    plt.close(fig)
...
def callback(x):
    ...
    partial_results = {
        ...
        "iota_weight": IOTA_WEIGHT,
        "current_weight": CURRENT_WEIGHT,
        "plasma_current_filament_A": float(plasma_current.current.get_value()),
        "plasma_current_filament_R_m": float(plasma_filament_radius),
        "plasma_current_filament_I0": float(I0_boozer),
...
if SPARSE:
    ...
    # Always write non-sparse runs into the finite-current output tree so
    # initializations from single_stage_epsilon_constraint stay separated.
    OUT_ROOT = os.path.join(
        "..",
        "single_stage_epsilon_constraint_finite_current",
...
# Add a fixed, user-specified plasma current filament at the plasma major radius.
plasma_filament_radius = float(surf.major_radius())
plasma_current_curve = CurvePlanarFourier(numquadpoints=128, order=1)
plasma_current_curve.set("rc(0)", plasma_filament_radius)
plasma_current_curve.set("X", 0.0)
plasma_current_curve.set("Y", 0.0)
plasma_current_curve.set("Z", 0.0)
plasma_current_curve.fix_all()
plasma_current = Coil(plasma_current_curve, Current(PLASMA_CURRENT))
plasma_current.current.fix_all()
print(
    f"Plasma current filament: I={plasma_current.current.get_value():.6e} A "
    f"at R={plasma_filament_radius:.6f} m, Z=0.0 m"
)

# Include filament in Biot-Savart field used by Boozer, but exclude from dipole current penalty.
coils = coils + [plasma_current]
bs = BiotSavart(coils)
...
I0_boozer = mu0 * plasma_current.current.get_value()
boozer_surface = initialize_boozer_surface(
    surf, mpol, ntor, bs, VOL_TARGET, CONSTRAINT_WEIGHT, IOTA_TARGET, G0, I0_boozer
)
...
for stage_idx, CURRENT_WEIGHT in enumerate(CURRENT_WEIGHT_SCHEDULE):
    ...
    plot_cross_section_with_plasma_filament(
    ...
    print(f"Plasma filament current: {plasma_current.current.get_value():.6e} A")
    print(f"Plasma filament Boozer I0: {I0_boozer:.6e}")
    print(f"Plasma filament cross-section marker: (R={plasma_filament_radius:.6f} m, Z=0.0 m)")
